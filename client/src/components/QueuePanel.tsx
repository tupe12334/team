  ...
  return (
    <section className="border border-[#1c2736] rounded-lg bg-[#0d1117] p-6 animate-fade-up" style={{ animationDelay: "60ms" }}>
      ...
      {loading ? (
        ...
        <div className="queue-panel__table-wrap overflow-x-auto">
          <table className="queue-table w-full text-left border-collapse">
            <thead className="queue-table__head">
              <tr className="queue-table__header-row border-b border-[#1c2736]">
              ...
              {tasks.map((task) => (
                <tr key={task.id} className="queue-table__row border-b border-[#1c2736]/50 hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="queue-table__td py-2.5">
                    <button onClick={() => { void handleDelete(task.id); }} disabled={deletingId === task.id}
                      className="queue-table__delete font-mono text-[10px] text-[#6e7681] hover:text-red-400 transition-colors disabled:opacity-40">
                      {deletingId === task.id ? "…" : "✕"}
                    </button>
                  </td>
                </tr>
