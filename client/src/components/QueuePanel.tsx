...
function EnqueueForm({ panel }: { panel: ReturnType<typeof useQueuePanel> }) {
  ...
  return (
    ...
    <form onSubmit={handleEnqueue} className="queue-panel__form flex gap-2 mb-5 flex-wrap items-center">
      ...
      <input type="number" value={priority || ""} onChange={(e) => { setPriority(Math.max(0, Math.trunc(Number(e.target.value)) || 0)); }} placeholder="priority" className={`${inputCls} w-24`} min={0} step={1} />
